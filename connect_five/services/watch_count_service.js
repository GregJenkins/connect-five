function watchCountService () 
{
    var self = this; 
    
    self.logWatchCount = function($rootScope) {
        $rootScope.$watch(function watchCount() { 
                var count = getWatchCount(); 
                console.log('Total watch count: ' + count); 
                return count; 
            }, function listenToWatchCount (newVal) { 
                console.log('Total updated watch coount: ' + newVal); 
        });
    }; 
    
    /*  
        Copy the following function from here: 
